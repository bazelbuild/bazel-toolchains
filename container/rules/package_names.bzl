...
"""Definitions of functions that return lists of package names.

This includes all package names for a given distro (debian jessie) as
well as private defs defining all packages for a given tool (a tool
requires several packages to be installed). This file also contains a
struct with the names of all tools. Lastly, this file defines a map from
tools to package names.
"""

# TODO(yiyu): merge this into //rules:package_names.bzl
def container_jessie_package_names():
  packages = []
  for tool in jessie_tools().values():
