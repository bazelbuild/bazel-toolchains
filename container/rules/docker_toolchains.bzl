...
"""Definitions of language_tool_layer, debian_pkg_tar and toolchain_container rules."""
...
IntermediateInfo = provider(fields = [
        "tars",
        "files",
        "env",
        "symlinks",
...
def _toolchain_container_impl(ctx):
    ...
    debian_output = _debian_pkg_tar_impl(ctx, download_pkgs_output_tar, download_pkgs_output_script)

    return _language_tool_layer_impl(
        ctx,
        symlinks = debian_output.providers[0].symlinks,
        env = debian_output.providers[0].env,
        tars = debian_output.providers[0].tars,
        files = debian_output.providers[0].files,
        packages = debian_output.providers[0].packages,
        additional_repos = debian_output.providers[0].additional_repos,
        keys = debian_output.providers[0].keys,
        installables_tars = debian_output.providers[0].installables_tars,
        installation_cleanup_commands = debian_output.providers[0].installation_cleanup_commands,
    )
...
def _debian_pkg_tar_impl(
    ...
    """Implementation for the debian_pkg_tar rule.

    debian_pkg_tar rule produces a tarball with all debian packages declared plus
    those in the language_tool_layer(s) this rule depends on.
    ...
    if ctx.file.installables_tar:
        ...
        download_pkgs_output_tar = ctx.actions.declare_file(download_pkgs_output_tar) or ctx.actions.declare_file(ctx.attr.name + ".tar")
        download_pkgs_output_script = ctx.actions.declare_file(download_pkgs_output_script) or ctx.actions.declare_file(ctx.attr.name + ".sh")
