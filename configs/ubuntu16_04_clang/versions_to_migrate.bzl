...
This test file is temporarily added to the repo while we migrate the toolchain
config release process from using the old to new versions.bzl file.
TODO(nlopezgi): remove this file once migration is complete
"""
...
def toolchain_config_specs():
    return [toolchain_config_spec1, toolchain_config_spec2]

DEFAULT_TOOLCHAIN_CONFIG_SPEC = toolchain_config_spec1

# Returns a dict with suppported Bazel versions mapped to the config version to use.
def bazel_to_config_spec_names():
    return {
        "0.20.0": ["8.0.0"],
        "0.21.0": ["8.0.0"],
        "0.22.0": ["8.0.0", "9.0.0"],
...
def rbe_repo_gen_spec():
    return struct(
        bazel_to_config_spec_names_map = bazel_to_config_spec_names,
